...
- 미션 구현을 완료한 후 GitHub을 통해 제출해야 한다.
  - 본인의 GitHub ID와 동일한 이름의 브랜치에 코드를 제출한다.
  - main Branch로 Pull Request를 생성한다.
...
- 자판기가 보유하고 있는 금액으로 동전을 무작위로 생성한다.[O]
  - 투입 금액으로는 동전을 생성하지 않는다. [O]
...
- [x] 보유금액 입력
  ...
  - [x] 10의 배수인지 검증
- [x] 동전 랜덤 생성
  - [x] 500, 100, 50, 10원 중 하나 선택
    - [x] 총 금액보다 동전금액이 크면 그 아래로 다시 선택
    - [x] 총 금액보다 동전금액이 작으면 총 금액에서 동전금액 빼고 다시진행
    - [x] 총 금액이 0이되면 종료
  - [x] 보유 동전 출력
...
- [x] 상품명, 가격, 수량을 입력 받는다
  - [x] 가격은 100원 이상 인지 검즘
  - [x] 가격이 10의 배수 인지 검증
- [x] ','로 상품명, 가격, 수량 구분. 개별 상품은 '[]'로 묶어 ';'로 구분
...
- [x] 구매할 상품 입력
  - [x] 상품이 존재하는지 검증
  - [x] 상품이 가진 돈보다 비싸면 다시 입력
- [x] 구입한 상품의 수량 -1
- [x] 투입 금액에서 상품 가격 삭감
- [x] 반복

- [x] 남은 돈이 제일 싼 상품보다 적으면 잔돈 돌려줌
- [ ] 상품이 다 떨어지면 잔돈을 돌려줌
